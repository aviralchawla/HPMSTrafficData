...
def correct_hpms_columns(hpms):
    ...
    hpms.rename(
        columns={"AADT_COMBINATION": "AADT_HDV", "AADT_SINGLE_UNIT": "AADT_MDV"},
        inplace=True,
    )
    ...
    states_info = pd.DataFrame(
        [(state.fips, state.name) for state in us.states.STATES],
        columns=["STATEFP", "STATENAME"],
    )
...
def merge_uac_data(hpms, hpms_uac):
    """
    Summary: This function merges the HPMS dataset with the urban area codes dataset to add urban area codes to the road links in the network.
    Inputs:
      -hpms (DataFrame) - The HPMS dataset
      -hpms_uac (DataFrame) - The HPMS urban area codes dataset
    Outputs:
      -hpms (DataFrame) - The HPMS dataset with urban area codes added
    """

    hpms_uac = hpms_uac[["FID_Link_Cnty_Intxn", "UACE10", "UATYP10"]]
    hpms = hpms.merge(hpms_uac, on="FID_Link_Cnty_Intxn", how="left")
    hpms["NEW_URBAN_CODE"] = (
        hpms["UATYP10"].map({"U": hpms["UACE10"], "C": "99998"}).fillna("99999")
    )
...
def calculate_vkt_vmt(hpms):
    """
    Summary: This function calculates the vehicle kilometers traveled (VKT) and vehicle miles traveled (VMT) for the HPMS dataset.
    Inputs:
      -hpms (DataFrame) - The HPMS dataset
    Outputs:
      -hpms (DataFrame) - The HPMS dataset with VKT and VMT calculated
    """
...
def subset_hpms(hpms):
    '''
    Summary: This function subsets the HPMS dataset to remove links with AADT values that are less than zero or greater than the sum of AADT_HDV and AADT_MDV.
    Inputs:
      -hpms (DataFrame) - The HPMS dataset
    Outputs:
      -hpms_sub (DataFrame) - The subsetted HPMS dataset
    '''
    ...
    sub_aadt_neg = hpms[
        (hpms["AADT"] > 0) & (hpms["AADT"] < (AADT_HDV_temp + AADT_MDV_temp))
    ]
    sub_aadt_errors = hpms[
        hpms["FID_Link_Cnty_Intxn"].isin(
            sub_aadt_nazero["FID_Link_Cnty_Intxn"].tolist()
            + sub_aadt_neg["FID_Link_Cnty_Intxn"].tolist()
        )
    ]
    hpms_sub = hpms[
        ~hpms["FID_Link_Cnty_Intxn"].isin(sub_aadt_errors["FID_Link_Cnty_Intxn"])
    ]
    ...
    hpms_sub = hpms_sub.apply(
        lambda col: col.replace([np.nan, np.inf, -np.inf], np.nan)
...
def main():
    ...
    hpms_intx = load_data(HPMS_DIR / 'HPMS.gdb', layer="HPMS_2018_county_intxn")
    hpms_uac = load_data(HPMS_DIR / 'HPMS.gdb', layer="HPMS_2018_cnty_uac_join")
    ...
    target_fields = (
        ...
        + [col for col in hpms_intx.columns if "AADT" in col]
        + ["Shape_Length"]
    )
    ...
    hpms_intx["LANE_KMS"] = hpms_intx["THROUGH_LANES"] * (
        hpms_intx["Shape_Length"] / 1000
    )
    hpms_intx["LANE_MILES"] = hpms_intx["THROUGH_LANES"] * (
        hpms_intx["Shape_Length"] / 1609.344
    )

    # helps us find links located in urban area codes
    hpms_intx = merge_uac_data(hpms_intx, hpms_uac)
    hpms_intx["ORIG_URBAN_CODE"] = [
        _.split(".")[0] for _ in hpms_intx["ORIG_URBAN_CODE"].astype(str)
    ]  # remove decimal points
    hpms_intx["NEW_URBAN_CODE"] = hpms_intx["NEW_URBAN_CODE"].astype(str)
    hpms_intx["UAC_IS_SAME"] = (
        hpms_intx["ORIG_URBAN_CODE"] == hpms_intx["NEW_URBAN_CODE"]
    ).astype(int)

    hpms_sub = subset_hpms(hpms_intx)

    hpms_sub.to_csv(HPMS_DIR / "hpms_aadt_subset.csv", index=False)
