...
class HPMSDataPreparation:
    ...
    def _make_td_gdb(self):
        ...
        print(
            f"Created file geodatabase for Traffic Density data: {Traffic_Density_gdb}"
        )
    ...
    def copy_raw_hpms(self, hpms_raw_gdb: Path):
        ...
        arcpy.Copy_management(
            f"{hpms_raw_gdb}\\{self.fc_123}", f"{self.hpms_gdb}\\{self.fc_123}"
        )
        arcpy.Copy_management(
            f"{hpms_raw_gdb}\\{self.fc_456}", f"{self.hpms_gdb}\\{self.fc_456}"
        )
    ...
    def copy_raw_census_counties(self, census_shp: Path):
        ...
        arcpy.conversion.ExportFeatures(
            in_features, out_features, where_clause, use_field_alias_as_name
        )
    ...
    def copy_raw_census_blocks(
        ...
        for fips in tqdm.tqdm(state_fips_codes):
            ...
            out_features = (
                f"{self.td_gdb}\\{os.path.basename(in_features).split('.')[0]}"
            )
            where_clause = ""
            use_field_alias_as_name = "NOT_USE_ALIAS"
            arcpy.conversion.ExportFeatures(
    ...
    def add_unique_id(self):
        ...
        arcpy.management.CalculateField(
            inFeatures, fieldName, expression, expression_type
        )
...
def main():
    ...
    prep.copy_raw_hpms(
        RAW_DIR / "ntad_2019_hpms_raw" / "NTAD2019_GDB_HPMS2018_2019_10_21.gdb"
    )
    prep.copy_raw_census_counties(
        RAW_DIR / "census" / "counties" / "tl_2020_us_county" / "tl_2020_us_county.shp"
    )
    prep.copy_raw_census_urban(
        RAW_DIR / "census" / "urban_areas" / "tl_2020_us_uac10" / "tl_2020_us_uac10.shp"
    )
