...
# Prepare storage directories for all processed data
STORAGE_DIR = Path("../data/processed_data")
if not STORAGE_DIR.exists():
    STORAGE_DIR.mkdir(parents=True)

HPMS_DIR = STORAGE_DIR / "HPMS"
if not HPMS_DIR.exists():
    HPMS_DIR.mkdir(parents=True)

TD_DIR = STORAGE_DIR / "Traffic_Density"
if not TD_DIR.exists():
    TD_DIR.mkdir(parents=True)


# Create file geodatabase for HPMS data
gdb_folder_path = str(HPMS_DIR.absolute())
gdb_name = "HPMS.gdb"
...
gdb_folder_path = str(TD_DIR.absolute())
...
file_paths = [base_file_path.format(fips, fips) for fips in state_fips_codes]
for file_path in tqdm.tqdm(file_paths):
    out_feature_class = f"{Traffic_Density_gdb}\\{os.path.basename(file_path).split('.')[0]}"
    where_clause = ""
    use_field_alias_as_name = "NOT_USE_ALIAS"
    arcpy.conversion.ExportFeatures(file_path, out_feature_class, where_clause, use_field_alias_as_name)
...
arcpy.env.workspace = str(HPMS_DIR.absolute())+"\\HPMS.gdb"
